...
namespace
{
  ...
  public:
    ...
    virtual void Send(const char* message, std::size_t size)
    {
      ...
      OStream os(RawChannel);
      os << hdr << algorithmHeader << sequence << OpcUa::Binary::RawMessage(message, size) << flush;
    }
  ...
  private:
    ...
    OpcUa::OpenSecureChannelResponse OpenChannel(std::shared_ptr<OpcUa::IOChannel> channel)
    {
      ...
      IOStream io(RawChannel);
      io << hdr << algorithmHeader << sequence << openChannel << flush;
